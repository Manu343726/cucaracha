...
func main() {
	...
	docsFile := flag.String("docs-out", "", "output file path for documentation")
	...
	pkg, _, err := reflect.ParsePackage(apiDir)
	if err != nil {
	...
	metadata, err := extractCommands(debuggerCommandsIface, pkg)
	if err != nil {
	...
	if err := generate(*outputFile, metadata, pkg); err != nil {
	...
	// Generate documentation file if path provided
	if *docsFile != "" {
		if err := generateDocumentation(*docsFile, metadata, pkg); err != nil {
...
type DebuggerCommandsMetadata struct {
	Commands      []CommandMetadata
	Documentation *docs.DocumentationIndex
}

// extractCommands extracts command metadata from interface methods using reflect API
func extractCommands(iface *reflect.Type, pkg *reflect.Package) (*DebuggerCommandsMetadata, error) {
	...
	docsIndex, err := docs.NewBuilder().Build(pkg)
	if err != nil {
		return nil, fmt.Errorf("failed to build documentation: %w", err)
	}

	return &DebuggerCommandsMetadata{
		Commands:      commands,
...
func generate(outputPath string, metadata *DebuggerCommandsMetadata, pkg *reflect.Package) error {
	file := codegen.NewFile("debugger").AddType(
		reflect.Struct(
			...
			}, utils.MapIf(metadata.Commands, func(cmd CommandMetadata) (*reflect.Field, bool) {
				if cmd.ArgsType == nil {
					return nil, false
				}
		...
		reflect.Struct(
			...
			}, utils.Map(metadata.Commands, func(cmd CommandMetadata) *reflect.Field {
		...
		&reflect.Enum{
			...
			Values: utils.Map(metadata.Commands, func(cmd CommandMetadata) *reflect.Constant {
...
`,
	...
	// Generate the main file content
	err := codegen.Generate(file, outputPath)
	if err != nil {
		return err
	}
...
func generateDocumentation(outputFile string, metadata *DebuggerCommandsMetadata, pkg *reflect.Package) error {
	// Since the documentation index is too big, we will encode to a binary file.
	...
	if err := encoder.Encode(metadata.Documentation); err != nil {
		return fmt.Errorf("failed to encode documentation: %w", err)
	}
