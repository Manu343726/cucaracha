...
// CommandInfo is an internal representation of a debugger command extracted from the DebuggerCommands interface
type CommandInfo struct {
	MethodName      string // e.g., "Step"
	CommandID       string // e.g., "DebuggerCommandStep"
	CommandName     string // e.g., "step" (camelCase for JSON)
	Comments        string // Documentation comment from interface
	ArgsType        string // e.g., "*StepArgs" (or "" if no args)
	ResultType      string // e.g., "*ExecutionResult"
	ResultFieldName string // e.g., "StepResult"
	ArgsFieldName   string // e.g., "cmd.StepArgs" (or "" if no args)
}
