...
func (r Registers) ReadAllRegisters() map[string]uint32 {
	result := make(map[string]uint32)
...
func (r Registers) ReadCPSR() uint32 {
	value, err := r.r.ReadByDescriptor(mc.Descriptor.Registers.CPSR)
	if err != nil {
		panic("failed to read CPSR register value: " + err.Error())
	}
	return value
}
