...
func (f *mockSourceFile) Name() string {
	return f.name
}

func (f *mockSourceFile) Path() string {
	return f.name
}

func (f *mockSourceFile) Snippet(r *sourcecode.Range) (*sourcecode.Snippet, error) {
	return &sourcecode.Snippet{
		Range: r,
		Lines: []*sourcecode.Line{
			{
...
func TestDecodeLocationExpr(t *testing.T) {
	...
	tests := []struct {
		name        string
		expr        []byte
		expectedLoc program.VariableLocation
		expectedNil bool
	}{
		{
		...
		// DW_OP_reg tests (0x50-0x6f)
		{
			name:        "DW_OP_reg0 (r0)",
			expr:        []byte{0x50},
			expectedLoc: program.RegisterLocation{Register: 16}, // r0 maps to 16
		},
		{
			name:        "DW_OP_reg1 (r1)",
			expr:        []byte{0x51},
			expectedLoc: program.RegisterLocation{Register: 17}, // r1 maps to 17
		},
		{
			name:        "DW_OP_reg9 (r9)",
			expr:        []byte{0x59},
			expectedLoc: program.RegisterLocation{Register: 25}, // r9 maps to 25
		},
		{
			name:        "DW_OP_reg13 (sp)",
			expr:        []byte{0x5D},
			expectedLoc: program.RegisterLocation{Register: 13}, // sp stays 13
		},
		{
			name:        "DW_OP_reg14 (lr)",
			expr:        []byte{0x5E},
			expectedLoc: program.RegisterLocation{Register: 14}, // lr stays 14
		},
		{
			name:        "DW_OP_reg15 (pc)",
			expr:        []byte{0x5F},
			expectedLoc: program.RegisterLocation{Register: 15}, // pc stays 15
		},
		// DW_OP_breg tests (0x70-0x8f) - base register + offset
		{
			name: "DW_OP_breg0 with positive offset",
			expr: []byte{0x70, 0x08}, // [r0 + 8]
			expectedLoc: program.MemoryLocation{
...
func TestLocationExprWithRegisterMapping(t *testing.T) {
	...
	tests := []struct {
		name    string
		expr    []byte
		checkFn func(t *testing.T, loc program.VariableLocation)
	}{
		{
			name: "DW_OP_reg0 should map to internal register 16",
			expr: []byte{0x50},
			checkFn: func(t *testing.T, loc program.VariableLocation) {
				regLoc, ok := loc.(program.RegisterLocation)
		...
		},
		{
			...
			expr: []byte{0x70, 0x04},
			checkFn: func(t *testing.T, loc program.VariableLocation) {
				memLoc, ok := loc.(program.MemoryLocation)
...
func TestVariableLocationTypes(t *testing.T) {
	...
	var loc program.VariableLocation

	// RegisterLocation
	loc = program.RegisterLocation{Register: 16}
	assert.NotNil(t, loc)
	regLoc, ok := loc.(program.RegisterLocation)
	assert.True(t, ok)
	...
	// MemoryLocation
	loc = program.MemoryLocation{BaseRegister: 13, Offset: 24}
	assert.NotNil(t, loc)
	memLoc, ok := loc.(program.MemoryLocation)
	assert.True(t, ok)
	...
	// ConstantLocation
	loc = program.ConstantLocation{Value: 42}
	assert.NotNil(t, loc)
	constLoc, ok := loc.(program.ConstantLocation)
	assert.True(t, ok)
...
func TestSourceLocationString(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "with column",
			loc: sourcecode.Location{
				File:   &mockSourceFile{name: "test.c"},
				Line:   42,
				Column: 10,
			},
		...
		},
		{
			name: "without column",
			loc: sourcecode.Location{
				File:   &mockSourceFile{name: "main.c"},
				Line:   100,
				Column: 0,
			},
			expected: "main.c:100",
		},
		{
			...
			loc: sourcecode.Location{
				File:   &mockSourceFile{name: "source.cpp"},
				Line:   1,
				Column: 1,
			},
...
func TestVariableInfoFields(t *testing.T) {
	v := program.VariableInfo{
		Name:        "myVar",
		TypeName:    "int",
		Size:        4,
		Location:    program.MemoryLocation{BaseRegister: 13, Offset: 24},
		IsParameter: false,
	}
	...
	memLoc, ok := v.Location.(program.MemoryLocation)
	assert.True(t, ok)
	assert.Equal(t, uint32(13), memLoc.BaseRegister)
	assert.Equal(t, int32(24), memLoc.Offset)
}
...
func TestFunctionDebugInfoFields(t *testing.T) {
	f := program.FunctionDebugInfo{
		Name:         "main",
		StartAddress: 0x10000,
		EndAddress:   0x10100,
		SourceFile:   "main.c",
		StartLine:    5,
		Parameters: []program.VariableInfo{
			{Name: "argc", TypeName: "int", Size: 4},
			{Name: "argv", TypeName: "char**", Size: 4},
		},
		LocalVariables: []program.VariableInfo{
...
func TestSourceLocationPropagation(t *testing.T) {
	...
	for i, entry := range entries {
		loc := &sourcecode.Location{
			File:   &mockSourceFile{name: entry.file},
			Line:   entry.line,
			Column: entry.column,
		}
	...
	for addr := uint32(0x100); addr < 0x110; addr += 4 {
		...
		assert.Equal(t, "test.c", loc.File.Name())
