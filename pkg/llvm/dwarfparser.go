...
func (p *DWARFParser) Parse() (*program.DebugInfo, error) {
...
func (p *DWARFParser) parseCompilationUnits() error {
	...
	var currentFunc *program.FunctionDebugInfo
	var scopeStack []*program.ScopeInfo
	...
	for {
		...
		case dwarf.TagSubprogram:
			// Function entry
			funcInfo := &program.FunctionDebugInfo{}
...
func (p *DWARFParser) parseVariable(entry *dwarf.Entry) *program.VariableInfo {
	varInfo := &program.VariableInfo{}
...
func (p *DWARFParser) parseLocation(entry *dwarf.Entry) program.VariableLocation {
	...
	case int64:
		return program.ConstantLocation{Value: loc}
	}
...
func (p *DWARFParser) decodeLocationExpr(expr []byte) program.VariableLocation {
	if len(expr) == 0 {
		return nil
	}
	...
	if op >= DW_OP_reg0 && op <= DW_OP_reg31 {
		...
		return program.RegisterLocation{Register: p.mapDWARFRegister(reg)}
	}
