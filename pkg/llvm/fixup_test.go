...
// These tests use program.Instruction and instruction descriptors to verify
...
func TestFixupEncodingCompatibleWithMOVIMM16L(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			instr := program.Instr(instructions.OpCode_MOV_IMM16L).
			...
			zeroImmInstr := program.Instr(instructions.OpCode_MOV_IMM16L).
				U(0).
				NamedR(tt.register).
				MustBuild()
...
func TestFixupEncoding32BitAddressWithMOVInstructions(t *testing.T) {
	...
	for _, targetAddr := range testAddresses {
		t.Run(fmt.Sprintf("addr_%08X", targetAddr), func(t *testing.T) {
			...
			// Build MOVIMM16L instruction using program.Instr
			loInstr := program.Instr(instructions.OpCode_MOV_IMM16L).
				U(loImm).
				NamedR(register).
				MustBuild()
...
func TestFixupEncodingBitFieldsDoNotOverlap(t *testing.T) {
	...
	zeroInstr := program.Instr(instructions.OpCode_MOV_IMM16L).
		U(0).
		NamedR("r7").
		MustBuild()
...
func TestFixupEncodingWithRealCucarachaOpcodes(t *testing.T) {
	// Test with actual Cucaracha instructions built using program.Instr
