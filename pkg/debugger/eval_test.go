...
func TestEvalMemory(t *testing.T) {
	...
	state := backend.Runner().Debugger().State()
	state.WriteByte(0x100, 0x78)
	state.WriteByte(0x101, 0x56)
	state.WriteByte(0x102, 0x34)
	state.WriteByte(0x103, 0x12) // Value at 0x100 = 0x12345678

	state.WriteByte(0x1000, 0xEF)
	state.WriteByte(0x1001, 0xBE)
	state.WriteByte(0x1002, 0xAD)
	state.WriteByte(0x1003, 0xDE) // Value at sp = 0xDEADBEEF
