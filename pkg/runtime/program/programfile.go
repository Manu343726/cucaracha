...
func (f *Function) FirstInstructionIndex() int {
	if len(f.InstructionRanges) == 0 {
		return -1
	}

	return f.InstructionRanges[0].Start
}
...
func Resolve(pf ProgramFile, memoryLayout *memory.MemoryLayout) (ProgramFile, error) {
	if memoryLayout == nil {
		return nil, fmt.Errorf("memory layout must be provided for resolution")
	}
...
//
// If debug info is available, it is used to find the function using the original source code function name.
// Otherwise, the function name is matched against the assembly-level function names (which may be mangled).
func FunctionByName(pf ProgramFile, name string) (*Function, error) {
	if pf.MemoryLayout() == nil {
		return nil, fmt.Errorf("program file memory addresses are not resolved")
	}

	// First, try to find function using debug info (source-level names)
	debugInfo := pf.DebugInfo()
	if debugInfo != nil {
		if fn, ok := debugInfo.Functions[name]; ok {
			// Synthesize Function metadata from debug info
			return &Function{
				Name:       name,
				...
				EndLine:    fn.EndLine,
				InstructionRanges: []InstructionRange{
					{Start: int(fn.StartAddress-pf.MemoryLayout().CodeBase) / 4, Count: int(fn.EndAddress-fn.StartAddress) / 4},
				},
			}, nil
		}
	}

	// Fallback: search assembly-level function names
...
func GlobalByName(pf ProgramFile, name string) (*Global, error) {
	if pf.MemoryLayout() == nil {
		return nil, fmt.Errorf("program file memory addresses are not resolved")
	}

	// TODO: Use debug info if available to find original source-level global names

	// Fallback: search assembly-level global names
