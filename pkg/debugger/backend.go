...
// NewBackendWithLayout creates a new debugger backend with a specific memory layout.
// This allows full control over memory regions (system descriptor, vector table,
// code, data, heap, stack, peripherals).
func NewBackendWithLayout(memorySize uint32, layout *peripheral.MemoryLayout) *Backend {
	return &Backend{
		runner:    interpreter.NewRunnerWithLayout(memorySize, layout),
		addrToIdx: make(map[uint32]int),
		idxToAddr: make(map[int]uint32),
	}
}
...
func (b *Backend) GetCallStack() []StackFrame {
	...
	memSize := uint32(state.MemorySize())
	...
	for i := uint32(0); i < maxStackScan && len(frames) < maxFrames; i += 4 {
		...
		potentialRA := uint32(state.ReadByte(addr)) |
			uint32(state.ReadByte(addr+1))<<8 |
			uint32(state.ReadByte(addr+2))<<16 |
			uint32(state.ReadByte(addr+3))<<24
