...
func TestBinaryFileParser_MemoryLayoutIsCorrect(t *testing.T) {
	...
	assert.Equal(t, uint32(0), layout.CodeBase, "code base should be 0")
}
...
func TestBinaryFileParser_LLVMGeneratedObjectFile(t *testing.T) {
	...
	runtimeMemoryLayout := &memory.MemoryLayout{
		CodeBase: 0x10000000,
		CodeSize: 0x01000000,
		DataBase: 0x20000000,
		DataSize: 0x01000000,
	...
	resolved, err := program.Resolve(pf, runtimeMemoryLayout)
	...
	assert.Equal(t, runtimeMemoryLayout, resolvedLayout, "resolved memory layout should match runtime layout")
}
...
func TestBinaryFileParser_LLVMGeneratedObjectFile_CompareWithAssembly(t *testing.T) {
	...
	resolvedBinary, errBinary := program.Resolve(binaryPF, runtimeMemoryLayout)
	resolvedAssembly, errAssembly := program.Resolve(assemblyPF, runtimeMemoryLayout)
	...
	require.NotNil(t, binLayout, "binary memory layout should be set")
	require.NotNil(t, asmLayout, "assembly memory layout should be set")

	assert.Equal(t, runtimeMemoryLayout, binLayout, "binary memory layout should match runtime layout")
	assert.Equal(t, runtimeMemoryLayout, asmLayout, "assembly memory layout should match runtime layout")
}
