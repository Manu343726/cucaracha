...
type relocationEntry struct {
	...
	Usage      program.SymbolReferenceUsage // Lo or Hi part of the address
}
...
type BinaryFile struct {
	...
	functionsMap    map[string]program.Function
	instructionsAll []program.Instruction
	globalsValue    []program.Global
	labelsValue     []program.Label
	memoryLayout    *memory.MemoryLayout
	debugInfo       *program.DebugInfo
}

// ParseBinaryFile parses a .o ELF file and returns a ProgramFile
func ParseBinaryFile(path string) (program.ProgramFile, error) {
...
func (p *BinaryFileParser) Parse() (*BinaryFile, error) {
	...
	result := &BinaryFile{
		...
		functionsMap:    make(map[string]program.Function),
		instructionsAll: []program.Instruction{},
		globalsValue:    []program.Global{},
		labelsValue:     []program.Label{},
	}
	...
	for i := 0; i < numInstructions; i++ {
		...
		if len(instrRelocs) > 0 {
			...
			if instrRelocs[0].Usage == program.SymbolUsageLo {
		...
		if err != nil {
			...
			result.instructionsAll = append(result.instructionsAll, program.Instruction{
				LineNumber: i + 1,
		...
		var symbolRefs []program.SymbolReference
		for _, rel := range instrRelocs {
			symbolRefs = append(symbolRefs, program.SymbolReference{
				Name:  rel.SymbolName,
				Usage: rel.Usage,
			})
		}
	...
	for addr, name := range functionSymbols {
		...
		if instrIdx >= 0 {
			...
			result.functionsMap[name] = program.Function{
				Name: name,
				InstructionRanges: []program.InstructionRange{
	...
	for addr, name := range labelSymbols {
		...
		result.labelsValue = append(result.labelsValue, program.Label{
			Name:             name,
	...
	for _, section := range elfFile.Sections {
		...
		if section.Name == ".data" || section.Name == ".rodata" || section.Name == ".bss" {
			...
			for name, sym := range globalDataSymbols {
				...
				result.globalsValue = append(result.globalsValue, program.Global{
					Name:        name,
					Size:        size,
					InitialData: initData,
					Type:        program.GlobalObject,
				})
			}
		}
	}
	...
	result.memoryLayout = &memory.MemoryLayout{
		CodeBase:  uint32(textSection.Addr),
		TotalSize: codeSize + dataSize,
		CodeSize:  codeSize,
		DataSize:  dataSize,
		DataBase:  uint32(textSection.Addr) + codeSize,
	}
...
func (p *BinaryFileParser) parseRelocations(elfFile *elf.File, textSection *elf.Section, symbols []elf.Symbol, labelSymbols map[uint32]string) ([]relocationEntry, map[uint32]string, []program.Global) {
	var relocations []relocationEntry
	generatedLabels := make(map[uint32]string)
	var rodataGlobals []program.Global
		...
		if funcSym, ok := functionSymbols[pair.symIndex]; ok {
			...
			if sectInfo.section != textSection {
				...
				if existingName, ok := targetSymbols[targetAddr]; ok {
					...
					rodataGlobals = append(rodataGlobals, program.Global{
						Name:        symbolName,
						Size:        len(initData),
						InitialData: initData,
						Type:        program.GlobalObject,
