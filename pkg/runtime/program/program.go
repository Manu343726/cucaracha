...
func Reg(name string) (instructions.OperandValue, error) {
	reg, err := mc.Descriptor.RegisterClasses.RegisterByName(name)
	if err != nil {
...
func (b *InstructionBuilder) R(index int) *InstructionBuilder {
	reg, err := mc.Descriptor.RegisterClasses.Class(registers.RegisterClass_GeneralPurposeInteger).Register(index)
