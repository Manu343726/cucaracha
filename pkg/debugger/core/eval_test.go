...
func mockRuntime(t *testing.T) runtime.Runtime {
	config := system.SystemConfig{
		...
		Interrupts: system.InterruptConfig{
			Vectors: 32,
		},
	}

	systemDescriptor, err := config.Setup()
	require.NoError(t, err, "failed to create system descriptor")
	interpreter, err := interpreter.NewInterpreter(systemDescriptor)
	require.NoError(t, err, "failed to create interpreter")
	return interpreter
}

func mockProgramFile(t *testing.T) program.ProgramFile {
	return &program.ProgramFileContents{
		FileNameValue:   "program.cucaracha",
		SourceFileValue: "program.c",
		FunctionsValue: map[string]program.Function{
			"fragmented": {
				Name:       "fragmented",
		...
		InstructionsValue: []program.Instruction{
			{Text: "MOVIMM16L #1, r0"},
			{Text: "JMP r4, r5"},
			{Text: "NOP"},
			{Text: "NOP"},
...
func TestEvalArithmetic(t *testing.T) {
	eval := NewExpressionEvaluator(mockRuntime(t), mockProgramFile(t))
...
func TestEvalRegisters(t *testing.T) {
	...
	eval := NewExpressionEvaluator(runtime, mockProgramFile(t))

	// Set up some register values using Backend methods
	require.NoError(t, cpu.WriteRegisterByName(runtime.CPU().Registers(), "r0", 100))
	require.NoError(t, cpu.WriteRegisterByName(runtime.CPU().Registers(), "r1", 200))
	require.NoError(t, cpu.WriteRegisterByName(runtime.CPU().Registers(), "sp", 0x1000))
	require.NoError(t, cpu.WriteRegisterByName(runtime.CPU().Registers(), "lr", 0x2000))
	require.NoError(t, cpu.WriteRegisterByName(runtime.CPU().Registers(), "pc", 0x3000))
	tests := []struct {
		name     string
		expr     string
		expected uint32
...
func TestEvalMemory(t *testing.T) {
	...
	require.NoError(t, memory.WriteUint32(runtime.Memory(), 0x100, 0x12345678))
	require.NoError(t, memory.WriteUint32(runtime.Memory(), 0x1000, 0xDEADBEEF))
